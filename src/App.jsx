...
export default function App() {
  const [account, setAccount] = useState(load("itp_account", {
    ...
    riskPercent: 1.0,
    pair: "XAUUSD",
    tickValue: 1, // profit per point per lot (user adjustable)
    pointSize: 0.1, // e.g., gold tick size
  }));
  ...
  const [idea, setIdea] = useState(load("itp_idea", {
    ...
    atr: "", // optional for SL sanity-check
  }));
  ...
  const positionSizing = useMemo(() => {
    ...
    const valuePerLotPerPoint = num(account.tickValue) || 1; // user-provided
      ...
      .concat(
        rows.map((r) =>
          [
            ...
            fmt(r.lot),
            r.atr ?? "",
            r.news?.highImpactToday ? "Y" : "N",
            r.news?.within30m ? "Y" : "N",
            r.news?.avoidNews ? "Y" : "N",
            quote(r.notes ?? ""),
          ].join(",")
        )
      )
  ...
  const clearAll = () => {
    if (confirm("Clear all saved ideas?")) setRows([]);
  };
      ...
      <div className="mx-auto max-w-7xl p-4 md:p-8">
        <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            <button onClick={exportCSV} className="px-4 py-2 rounded-2xl bg-slate-800 hover:bg-slate-700 shadow">
              Export CSV
            </button>
            <button onClick={clearAll} className="px-4 py-2 rounded-2xl bg-rose-700/80 hover:bg-rose-600 shadow">
              Clear Ideas
            </button>
          </div>
        </header>
        ...
        <section className="mt-6 grid md:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <Field label="Pair">
                <input
                  ...
                  className="input"
                  value={account.pair}
                  onChange={(e) => setAccount({ ...account, pair: e.target.value })}
                />
              </Field>
              <Field label="Point Size">
                <input
                  ...
                  className="input"
                  step="0.001"
                  value={account.pointSize}
                  onChange={(e) => setAccount({ ...account, pointSize: e.target.value })}
                />
              </Field>
              <Field label="Value / point / lot">
                <input
                  type="number"
                  className="input"
                  step="0.01"
                  value={account.tickValue}
                  onChange={(e) => setAccount({ ...account, tickValue: e.target.value })}
                />
              </Field>
            </div>
          ...
          <Card title="News Guard">
            <div className="grid grid-cols-2 gap-3">
              <Toggle
                label="High-impact news today"
                checked={idea.news.highImpactToday}
                onChange={(v) => setIdea({ ...idea, news: { ...idea.news, highImpactToday: v } })}
              />
              <Toggle
                label="Within 30 minutes"
                checked={idea.news.within30m}
                onChange={(v) => setIdea({ ...idea, news: { ...idea.news, within30m: v } })}
              />
              <Toggle
                label="Avoid trading during news"
                checked={idea.news.avoidNews}
                onChange={(v) => setIdea({ ...idea, news: { ...idea.news, avoidNews: v } })}
              />
            </div>
            <p className="mt-3 text-xs text-slate-400">
              Pro tip: if <b>Within 30 minutes</b> and <b>Avoid trading during news</b> are both on, skip entries until volatility settles.
            </p>
          </Card>
        </section>
        ...
        <Card title="Idea Builder">
          <div className="grid md:grid-cols-6 gap-3">
            ...
            <Field label="Pair">
              <input
                ...
                className="input"
                value={idea.pair}
                onChange={(e) => setIdea({ ...idea, pair: e.target.value })}
              />
            </Field>
            <Field label="Bias (D1/H4)">
              <input
                type="text"
                className="input"
                placeholder="Uptrend / Downtrend / Range"
                value={idea.biasTF}
                onChange={(e) => setIdea({ ...idea, biasTF: e.target.value })}
              />
            </Field>
            <Field label="ATR (optional)">
              <input
                ...
                step="0.01"
                value={idea.atr}
                onChange={(e) => setIdea({ ...idea, atr: e.target.value })}
              />
            </Field>
            <Field className="md:col-span-2" label="Zones / S&D / Fib levels">
              <input
                type="text"
                className="input"
                value={idea.zones}
                onChange={(e) => setIdea({ ...idea, zones: e.target.value })}
              />
            </Field>
          </div>
          ...
          <div className="mt-4 grid md:grid-cols-6 gap-3">
            <Field label="Entry">
              <input
                ...
                className="input"
                value={idea.entry}
                onChange={(e) => setIdea({ ...idea, entry: e.target.value })}
              />
            </Field>
            <Field label="Stop Loss">
              <input
                ...
                className="input"
                value={idea.sl}
                onChange={(e) => setIdea({ ...idea, sl: e.target.value })}
              />
            </Field>
            <Field label="Take Profit">
              <input
                ...
                className="input"
                value={idea.tp}
                onChange={(e) => setIdea({ ...idea, tp: e.target.value })}
              />
            </Field>
            <Stat label="R : R" value={rr ? rr.toFixed(2) : "-"} />
            <Stat label="Lot (calc)" value={fmt(positionSizing.lot)} />
            <div className="flex items-end">
              <button onClick={addIdea} className="w-full h-10 rounded-2xl bg-emerald-600 hover:bg-emerald-500 shadow">
                Add Idea
              </button>
            </div>
          </div>
          ...
          <Field className="mt-4" label="Notes">
            <textarea
              ...
              placeholder="Entry trigger on M5: bullish engulfing at demand + EMA20>EMA50. Trail below swing lows."
            />
          </Field>
        </Card>
        ...
        <Card title="Journal">
          <div className="flex items-center justify-between gap-3 mb-3">
            <input
              placeholder="Filter by pair / note / date"
                ...
                {rows
                  .filter((r) =>
                    (filter || "")
                      ...
                      .split(" ")
                      .filter(Boolean)
                      .every((t) => JSON.stringify(r).toLowerCase().includes(t))
                  )
                  .map((r) => (
                    <tr key={r.id} className="border-t border-white/5 hover:bg-white/5">
                      ...
                      <td className="py-2 pr-3 text-xs">
                        {r.news?.highImpactToday ? "High" : "-"}/{r.news?.within30m ? "30m" : "-"}
                      </td>
                      <td className="py-2 pr-3 max-w-[28ch] truncate" title={r.notes}>{r.notes}</td>
                      <td className="py-2 pr-3">
                        <button
                          className="px-2 py-1 text-xs rounded-xl bg-slate-800 hover:bg-slate-700"
        ...
        <footer className="mt-8 text-center text-xs text-slate-500">
          Built for fast daily idea creation • Local-only (saves in your browser) • v1.0
        </footer>
      </div>
    </div>
  );
...
function Card({ title, children }) {
  return (
    <motion.section
      ...
      animate={{ opacity: 1, y: 0 }}
      className="bg-slate-900/60 backdrop-blur rounded-2xl p-4 md:p-6 shadow-xl border border-white/10"
    >
...
function Stat({ label, value }) {
  return (
    <div className="flex flex-col justify-end bg-slate-800/60 rounded-2xl p-3">
...
function Toggle({ label, checked, onChange }) {
  return (
    <button
      ...
      className={`flex items-center justify-between w-full h-10 px-3 rounded-2xl border border-white/10 bg-slate-800/40 hover:bg-slate-800 ${
        checked ? "ring-1 ring-emerald-400/30" : ""
      }`}
    >
